...
async function runTests() {
    ...
    try {
        ...
        if (res.status === 200 && data.token) {
            ...
            console.log(`[PASS] POST /auth/login (Analyst) => HTTP ${res.status}, role: ${data.user.role}`);
        } else {
    ...
    try {
        ...
        if (res.status === 200 && data.token) {
            ...
            console.log(`[PASS] POST /auth/login (Principal) => HTTP ${res.status}, role: ${data.user.role}`);
        } else {
    ...
    if (analystToken) {
        try {
            ...
            if (res.status === 200 && Array.isArray(schoolsList) && schoolsList.length > 0) {
                realSchoolId = schoolsList[0].school_id;
                if (schoolsList.length > 1) {
                    otherSchoolId = schoolsList[1].school_id;
                }
                console.log(`[PASS] GET /schools => HTTP ${res.status}, found ${schoolsList.length} schools. First School ID: ${realSchoolId}`);
            } else {
    ...
    if (analystToken && realSchoolId) {
        try {
            ...
            const data = await res.json();
            if (res.status === 200 && data.projectedCompositeMin !== undefined) {
                console.log(`[PASS] POST /simulations => HTTP ${res.status}, projectedCompositeMin: ${data.projectedCompositeMin}`);
            } else {
                console.log(`[FAIL] POST /simulations => HTTP ${res.status}, body: ${JSON.stringify(data)}`);
            }
        } catch (e) {
            console.log(`[FAIL] POST /simulations => Error: ${e.message}`);
        }
    }
    ...
    if (realSchoolId) {
        try {
            ...
            if (parts.length === 3) {
                ...
                const res = await fetch(`${BASE_URL}/alerts`, { // Endpoint requiring auth
    ...
    // A Parent token (or Principal token) trying to access Analyst routes (e.g. GET /alerts or GET /audit-logs)
    if (parentToken) {
        try {
            ...
            if (res.status === 403 || res.status === 401) {
                ...
                const data = await res.json();
                console.log(`[VULN] Privilege Escalation (Parent -> Analyst Route) => Parent token allowed access to GET /alerts! HTTP ${res.status}, body: ${JSON.stringify(data)}`);
            }
        } catch (e) {
